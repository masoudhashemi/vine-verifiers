...
def _set_cuda_visible_from_device_arg(vllm_device: str) -> None:
    ...
    # Only handle explicit cuda indices; leave 'cuda' as-is to respect current environment
    if dev.startswith("cuda:"):
        indices = dev.split(":", 1)[1].strip()
        if indices:
            # Normalize e.g. "0, 2" -> "0,2"
        ...
        # vLLM generally targets GPU; warn and do nothing (will likely error if no GPU)
...
def init_vllm(model_name, vllm_device, vllm_gpu_memory_utilization, vllm_dtype, block_size, vllm_max_seqs=None):
    ...
    with world_size_patch, profiling_patch:
        # Build kwargs dynamically based on installed vLLM EngineArgs support
        import inspect
        try:
        ...
        try:
            ...
            # Fallback for older/newer vLLM arg names
