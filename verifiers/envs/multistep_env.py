...
class MultiStepEnv(Environment):
    ...
    def step(self,
        ...
        def update_state(j, llm_response):
            ...
            if env_response_len > 0:
                # Make first token visible, rest use env_mask
                state["completion_mask"].append(1)  # First token always visible
                if env_response_len > 1:
                    state["completion_mask"].extend([self.env_mask] * (env_response_len - 1))
